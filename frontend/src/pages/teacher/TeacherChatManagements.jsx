...
import {
  MessageSquare, Users, LogOut, Send, Mic, MicOff,
...
const VoiceRecorder = ({ onSend, onCancel, dark }) => {
  ...
  const textColor = dark ? "#e8eaf6" : "#111827";
...
export default function TeacherChatManagement() {
  ...
  // Fetch teacher's chatrooms
  const fetchRooms = useCallback(async () => {
    setLoadingRooms(true);
    try {
      const res = await apiClient.get("/chat/chatrooms/teacher/");
      ...
      <div className="flex items-center justify-between px-4 py-3 flex-shrink-0" style={{ background: colors.surface, borderBottom: `1px solid ${colors.border}` }}>
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs" style={{ color: colors.text2 }}>{t("chat.subtitle")}</p>
          </div>
        </div>
        ...
        <div className={`fixed md:relative z-20 w-80 h-full transition-transform duration-300 ${sidebarOpen ? "translate-x-0" : "-translate-x-full"
          ...
          <div className="flex-1 overflow-y-auto">
            {loadingRooms ? (
              ...
              const roomTypeLabel = {
                parent_teacher: "Parent-Teacher", all_parents: "All Parents", all_teachers: "All Teachers",
                ...
                >
                  <div className="flex justify-between items-start">
                    <div className="flex-1">
                      ...
                      <p className="text-xs mt-1" style={{ color: colors.text2 }}>{roomTypeLabel}</p>
                    </div>
                    {room.unread_count > 0 && (
            ...
            <>
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-3">
                {loadingMessages ? (
                  ...
                  const isMine = msg.sender_role === "teacher";
                  return (
                    <div
                      key={msg.id}
                    ...
                    >
                      ...
                      {isMine && (
                        <div className="flex flex-col gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                          ...
                          <button onClick={() => handleDeleteMessage(msg)} className="p-1 rounded" style={{ background: colors.surface2 }} title="Delete">
