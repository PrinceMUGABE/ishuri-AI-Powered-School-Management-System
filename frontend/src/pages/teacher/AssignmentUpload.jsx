...
const TeacherAssignmentManagement = () => {
  ...
  }, [fAcademicYear, fTerm, fClassLevel, fSubject, fClassroom]);
  ...
  const StatsCards = () => (
    <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
      {[
        { label: 'Total Assignments', value: stats.total, color: 'from-gray-700 to-gray-900', icon: FileText },
        { label: 'Active', value: stats.active, color: 'from-green-500 to-green-700', icon: CheckCircle },
        { label: 'Overdue', value: stats.overdue, color: 'from-red-500 to-red-700', icon: AlertCircle },
        { label: 'Expired', value: stats.expired, color: 'from-yellow-500 to-yellow-700', icon: Clock },
  ...
  const StatusBadge = ({ status, isOverdue }) => {
    if (isOverdue && status === 'active') {
      return <span className="inline-flex px-2 py-1 rounded-full text-xs font-semibold bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-300">Overdue</span>;
    }
    const colors = {
    ...
    return <span className={`inline-flex px-2 py-1 rounded-full text-xs font-semibold ${colors[status] || colors.active}`}>{status}</span>;
  };
      ...
      <div className="space-y-5 p-6 bg-gray-50 dark:bg-gray-900 min-h-screen">
        ...
        <div className="flex justify-between items-center">
          <div>
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
              {teacherProfile?.full_name} · Upload and manage assignments for your classes
            </p>
          </div>
          <div className="flex items-center gap-3">
            <LanguageSwitcher />
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden">
          <div className="px-5 py-3.5 border-b border-gray-100 dark:border-gray-700 bg-gradient-to-r from-green-50 to-amber-50 dark:from-green-900/20 dark:to-amber-900/20 flex items-center justify-between flex-wrap gap-3">
            <div className="flex items-center gap-2">
              ...
              <h2 className="font-bold text-gray-800 dark:text-gray-200 text-sm">My Assignments</h2>
          ...
          {loadingAssignments ? (
            ...
            <div className="py-16 text-center">
              ...
              <p className="text-sm text-gray-400 dark:text-gray-500 font-medium">No assignments found</p>
              <p className="text-xs text-gray-300 dark:text-gray-600 mt-1">Upload your first assignment using the button above</p>
            </div>
          ) : (
            <>
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mt-3 text-xs">
                          <div>
                            <span className="text-gray-400 dark:text-gray-500">Due Date:</span>
                          ...
                          <div>
                            <span className="text-gray-400 dark:text-gray-500">Total Marks:</span>
                            <p className="font-medium text-gray-700 dark:text-gray-300">{assignment.total_marks || 'Not specified'}</p>
                          </div>
                          <div>
                          ...
                          <div>
                            <span className="text-gray-400 dark:text-gray-500">Teacher:</span>
              ...
              <div className="px-5 py-3 border-t border-gray-100 dark:border-gray-700 flex items-center justify-between flex-wrap gap-2">
                <div className="flex items-center gap-2 text-xs text-gray-500 dark:text-gray-400">
                  ...
                  <span>per page · {filteredAssignments.length} total</span>
                </div>
        ...
        {showUploadModal && (
          <ModalWrapper
            ...
            subtitle="Upload a PDF assignment for your students"
            ...
            <div className="space-y-4">
              ...
              >
                ...
                {uploadFile ? (
                  ...
                  <>
                    <p className="text-sm font-semibold text-gray-600 dark:text-gray-400">Click to select a PDF file</p>
                    <p className="text-xs text-gray-400 mt-1">Accepts .pdf only</p>
                  </>
                )}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-xs font-semibold text-gray-600 dark:text-gray-400 uppercase tracking-wider">Due Date</label>
                  <input
                    type="date"
                    value={dueDate}
                ...
                <div>
                  <label className="text-xs font-semibold text-gray-600 dark:text-gray-400 uppercase tracking-wider">Due Time</label>
                  <input
                    type="time"
                    value={dueTime}
              ...
              <div>
                <label className="text-xs font-semibold text-gray-600 dark:text-gray-400 uppercase tracking-wider">Total Marks</label>
                <input
                  type="number"
                  step="0.01"
                  value={totalMarks}
              ...
              <div>
                <label className="text-xs font-semibold text-gray-600 dark:text-gray-400 uppercase tracking-wider">Description</label>
                <textarea
                  value={description}
              ...
              <div>
                <label className="text-xs font-semibold text-gray-600 dark:text-gray-400 uppercase tracking-wider">Instructions</label>
                <textarea
                  value={instructions}
        ...
        {showDetailModal && selectedAssignment && (
          ...
          >
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-3">
                <InfoRow label="Subject" value={selectedAssignment.subject_name} icon={BookOpen} />
                <InfoRow label="Class" value={selectedAssignment.class_level_name} icon={GraduationCap} />
                <InfoRow label="Academic Year" value={selectedAssignment.academic_year_name} icon={CalendarIcon} />
                <InfoRow label="Teacher" value={selectedAssignment.teacher_name} icon={UserCheck} />
                <InfoRow label="Status" value={selectedAssignment.status} icon={Activity} />
                <InfoRow label="Due Date" value={selectedAssignment.due_date ? new Date(selectedAssignment.due_date).toLocaleDateString() : 'Not set'} icon={CalendarIcon} />
                {selectedAssignment.due_time && <InfoRow label="Due Time" value={selectedAssignment.due_time} icon={ClockIcon} />}
                {selectedAssignment.total_marks && <InfoRow label="Total Marks" value={selectedAssignment.total_marks} icon={Award} />}
              </div>
              ...
              {selectedAssignment.instructions && (
                <div className="bg-gray-50 dark:bg-gray-800 rounded-xl p-4">
                  <p className="text-xs font-semibold text-gray-600 dark:text-gray-400 uppercase tracking-wider mb-2">Instructions</p>
        ...
        {showEditModal && editingAssignment && (
          ...
          >
            <div className="space-y-4">
              <div>
                <label className="text-xs font-semibold text-gray-600 dark:text-gray-400 uppercase tracking-wider">Title</label>
                <input
                  type="text"
                  value={editTitle}
            ...
            <div className="space-y-4">
              <div className="bg-red-50 dark:bg-red-900/20 rounded-xl p-4">
                ...
                <p className="text-xs text-red-600 dark:text-red-400 mt-2">
                  This will permanently remove this assignment and all its data from the system.
                </p>
              </div>
